...
def initialize():
    ...
    course_df = pd.read_csv( str(data_dir/"courses.csv") )
    user_plog = {
    ...
    return user_plog, course_feat

user_plog, course_feat = initialize()
...
def get_pubtime_distribution():
    ...
    month_prob = np.mean( year_prob[1:-1,:], axis=0)

    return Counter(res), month_prob

def draw_pubtime_distribution():
    pubtime_cnt, month_prob = get_pubtime_distribution()

    def time_to_int( t, year_offset = 2014, mon_offset = 12 ):
        year = int(t[:4])
        month = int(t[-2:])
        return (year - year_offset) * 12 + (month - mon_offset)
    ...
    prob = np.zeros( (len(pubtime, )) )
    for i in range( 0, prob.shape[0], 12 ):
        prob[i:i+12] = month_prob
...
def get_course_prob():
    _, prob_func = get_price_distribution()
    _, month_prob = get_pubtime_distribution()
    course_prob = {}
    ...
    for cid, feat in course_feat.items():
        try:
            ...
            y = int(time[:4])
            m = int(time[-2:])-1
            course_prob[cid] = prob_func( feat['course_price'] ) * month_prob[m] * year_weight(y)
...
if __name__ == "__main__":
    ...
    for name in test_files:
        ...
        print(f"Inference {file_path.stem}")
        output_path = generate_course_pred_csv( file_path, courses, prob, output_dir )

    from utils.submission_format import course_to_topic_pred

    course_to_topic_pred(
        "../output/month_price_sample/test_seen_pred.csv",
        "../data/courses.csv",
        "../data/subgroups.csv",
    ).to_csv("../output/month_price_sample/test_seen_group_pred.csv", index=False)
    course_to_topic_pred(
        "../output/month_price_sample/test_unseen_pred.csv",
        "../data/courses.csv",
        "../data/subgroups.csv",
    ).to_csv("../output/month_price_sample/test_unseen_group_pred.csv", index=False)
